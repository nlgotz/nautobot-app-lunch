...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_LUNCH_xxx
namespace = Collection("nautobot_lunch")
namespace.configure(
    {
        "nautobot_lunch": {
            "nautobot_ver": "2.0.0",
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_lunch.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_lunch.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_lunch.nautobot_ver,
        "PYTHON_VER": context.nautobot_lunch.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_lunch.project_name}",
        f'--project-directory "{context.nautobot_lunch.compose_dir}"',
    ]

    for compose_file in context.nautobot_lunch.compose_files:
        compose_file_path = os.path.join(context.nautobot_lunch.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_lunch.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_lunch.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_lunch"
...
@task(
    help={
        "version": "Version of Nautobot Lunch to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_lunch'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_lunch/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_lunch.local):
